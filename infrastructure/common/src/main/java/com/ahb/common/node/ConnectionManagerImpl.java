...
public class ConnectionManagerImpl implements ConnectionManager {
    ...
    public void init() {
        try {
            ...
            handler = new AhbExchangeHandler(this);
            ...
            tServer = new TThreadedSelectorServer(new TThreadedSelectorServer.Args(nonblockingServerTransport).processor(tProcessor));
    ...
    public ExchangeInfo exchange(ExchangeInfo input) throws TException {
        switch (input.getEType()) {
            case HEARTBEAT:
                LOGGER.info("heart beat received " + input.getFromNode() + input.getToken());
                return input;
            case BIZ_DIS:
    ...
    public JoinResp requestJoin(JoinReq req) throws TException {
        LOGGER.info("Exchanging info..." + req);
        synchronized (JOIN_REQUESTS) {
            ...
            if (reqCnt != null) {
                ...
                LOGGER.info("Ignore request for " + req);
                return ignoreResp;
            } else {
    ...
    public void cleanToken(ExchangeInfo exchangeInfo) throws TException {
        LOGGER.info("Token clean up " + exchangeInfo);
